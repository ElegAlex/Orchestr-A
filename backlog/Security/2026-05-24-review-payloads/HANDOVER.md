...
## Current state
- **Master HEAD ahead of prod by 1 runtime delta: SEC-031 `198160f`** (users-list scope/payload, code-only, no migration). Everything else since prod (`ce0c729`) is fixture/CI/spec/docs — zero additional runtime delta.
- **Prod deployed HEAD: `ce0c729`** (5-delta code-only micro-deploy, 2026-05-29). DB at 56 migrations, health OK.
...
## Next
TST-MTX-001 (`81da4c9`) and TOOL-COH-003 (`bb89f40`) closed — both test-fixture/docs-only, zero runtime delta. Nothing in flight. Open work = deploy SEC-031 standalone (`198160f`, the one undeployed runtime delta since prod `ce0c729`), or pick from the remaining TODO filings / later phases (operator decides).
