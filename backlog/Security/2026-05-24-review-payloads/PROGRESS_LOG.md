...
## 2026-05-28 — COR-001 closed (ADMIN role-code bypass → projects:manage_any permission check) — Phase 4 opening
- **Session ID:** 2026-05-28-cor-001
- **Tasks closed:** COR-001 (Phase 4, Cluster B, claude-only, severity important, correctness·logic). **First Phase 4 task** — opens the RBAC-complétude arc on the `sinon COR-001/002` branch (TST-001 confirmed test-fixture+meta-test at kickoff).
- **Tasks moved to BLOCKED:** none.
- **Commits:** `21e3d37` (in_progress), `cb3b5e1` (fix — `[closes COR-001]`), `<pending>` (closeout).
- **Counter:** checked-set **54 → 55** (script raw DONE **55 → 56**).
- **Pre-flight:** confirmed no IN_PROGRESS leftover post-DOC-001 (`006adb7`/`dd1381f`); master HEAD `dd1381f`, prod functionally current at `ebcd9e1`. Target confirmed at `epics.service.ts:107` `if (userRole === 'ADMIN') return;` (no line drift from kickoff's 107). Canonical mirror `projects.service.ts:77` (`permissions.includes('projects:manage_any')`) read verbatim. `PermissionsService.getPermissionsForRole(roleCode)` returns `Promise<readonly PermissionCode[]>` — `.includes('projects:manage_any')` typechecks. DAT-035 dead-code grep on method body: **NO** OWNER/LEAD literals against `project_members.role` (membership matched on `m.userId === userId`).
- **Wiring discovery (carry-forward for COR-002 + future RBAC tasks):** `RbacModule` is `@Global()`, so `PermissionsService` is injectable with zero module import (leaves/comments inject it with no RBAC import). The contract's anticipated "update EpicsModule" step was a no-op — `epics.module.ts` left untouched. Final file scope = 2 files (service + spec), not the 3 the contract reserved.
- **Diff scope (AC#6 — fix commit `cb3b5e1`):** 2 files, 83 insertions / 3 deletions — `epics.service.ts` (constructor DI of PermissionsService + permission-based bypass + docstring fix lines 99–101) + `epics.service.spec.ts` (mock PermissionsService provider + role-aware default mock + 2 witness tests). NO milestones.service.ts (COR-002 — separate session), NO projects.service.ts (read-only mirror reference).
- **FAIL-pre/PASS-post non-vacuous (AC#2):** witness = non-ADMIN role `DIRECTION_SI` whose resolved perms include `projects:manage_any`, user NOT in `project.members`. Pre-fix run (before service edit): FAILED with `ForbiddenException('Not a member of this project')` at `epics.service.ts:117` — membership fall-through, the right reason (not DI/instantiation error), 15/16 green. Post-fix: 16/16 green. The single thing letting `update` through is the manage_any bypass.
- **AC#4:** N/A — `assertProjectMembership` is a read-gate (assert, no mutation), not in the audit-sensitive list. No `audit_logs` entry. Pre-verified by contract, re-confirmed, not re-litigated.
- **Existing-test handling:** kept `'should bypass membership check for ADMIN role'`, adapted its mock (role-aware: `ADMIN → ['projects:manage_any']`, else `[]`) so its intent survives via the permission path. Member-passes regression stays covered by existing `'should allow member to update'` — no redundant duplicate added (advisor-flagged).
- **Gates:** `nest build` clean (typecheck gate — `tsc --noEmit` is RED on master by design, per memory); `pnpm test` 1692 → **1694** (+2: witness + negative) across 73 files / 6 turbo tasks; `pnpm test:e2e` turbo **4/4** (3 cached + web:build). Integration suite not re-run (service-layer auth-path change, no DB-surface change).
- **Deploy posture:** code-only change, no migration. **NOT auto-deployed** — operator's call whether to fold into a tooling deploy (could batch with COR-038 `24c6929`, still unshipped) or with the rest of Phase 4. Prod stays `ebcd9e1`; master now `cb3b5e1`+closeout, ahead of prod by COR-038 + COR-001.
- **INTERDIT DUAL-CLOSE respected:** COR-002 (milestones.service.ts:133, same mechanism verbatim) NOT touched — different file + different witness path → bundle criteria not met (HANDOVER learning #5 requires same file AND mechanism AND witness path). No shared-helper extraction. COR-002 takes its own session/prompt/commit.
- **Out of scope (next session):** COR-002 (milestones — mirror fix), then TST-001 + the 3 remaining Phase 4 RBAC tasks. HANDOVER refresh untouched (refresh = separate session).

## 2026-05-28 — COR-002 closed (ADMIN role-code bypass → projects:manage_any permission check) — Phase 4, COR-001 sibling
- **Session ID:** 2026-05-28-cor-002
- **Tasks closed:** COR-002 (Phase 4, Cluster B, claude-only, severity important, correctness·logic). Verbatim sibling of **COR-001** (`cb3b5e1`) — same anti-pattern in `MilestonesService.assertProjectMembership`.
- **Tasks moved to BLOCKED:** none.
- **Commits:** `f09386a` (in_progress), `27c0424` (fix — `[closes COR-002]`), `<pending>` (closeout).
- **Pre-flight:** confirmed no IN_PROGRESS leftover post-COR-001 (`cb3b5e1`/`e3865c3`); master HEAD `e3865c3`, prod functionally current at `ebcd9e1`. **Line reconciliation 133/144/139:** BACKLOG `:133`, kickoff `:144`; actual = `assertProjectMembership` declared line 139, `if (userRole === 'ADMIN') return;` line 144, stale docstring 135–137. Shape byte-identical to epics' method — no HALT. DAT-035 dead-code grep: **NO** OWNER/LEAD literals (membership matched on `m.userId === userId`, line 152), same as COR-001.
- **Wiring no-op (carry-forward #2 confirmed):** `milestones.module.ts` imports neither `RbacModule` nor `AuditModule` yet already injects `AuditPersistenceService` (both `@Global()`). `PermissionsService` injectable with zero module change. Module left untouched; file scope = 2 files (service + spec), not 3.
- **DIVERGENCE from COR-001 (advisor-flagged, not transcribed):** (1) **Member-passes had NO existing coverage** — milestones' `update`/`remove` tests pass no `currentUserId`, so they never reach `assertProjectMembership` (epics' did). Added a dedicated member-passes regression test (green pre- and post-fix). COR-001's "covered by existing test, no duplicate" learning is FALSE here. (2) **AC#4 N/A is path-specific** — milestones DOES import `AuditPersistenceService`/`emitDataExported` (OBS-026 `DATA_EXPORTED` on CSV export); N/A holds because the *changed method* `assertProjectMembership` is a read-gate, NOT because "no audit in file."
- **FAIL-pre/PASS-post non-vacuous (AC#2):** witness = non-ADMIN role `DIRECTION_SI` with resolved `projects:manage_any`, user NOT in `project.members`. Pre-fix run (spec edited, service unchanged): FAILED with `ForbiddenException('Not a member of this project')` at `milestones.service.ts:154:13` — membership fall-through, the right reason (not DI error), 32/33 green. Post-fix: 33/33 green.
- **Gates:** `nest build` clean (typecheck gate — confirms `getPermissionsForRole(userRole)` accepts `string | null | undefined`; `tsc --noEmit` is RED on master by design); `pnpm test` 1694 → **1697** (+3: bypass witness + negative + member-passes regression) across 73 files / 6 turbo tasks; `pnpm test:e2e` turbo **4/4**.
- **Deploy posture:** code-only change, no migration. **NOT auto-deployed.** Prod stays `ebcd9e1`; master now ahead by COR-038 (`24c6929`) + COR-001 (`cb3b5e1`) + COR-002 (`27c0424`) — **3 undeployed code changes** accumulated. HANDOVER refresh clearly due (separate session, NOT this one).
- **INTERDIT DUAL-CLOSE (inverse) respected:** COR-001 (`cb3b5e1`) NOT re-touched — no retroactive `[closes COR-001]`, no retroactive shared-helper extraction. Sibling-set (epics + milestones) complete; no phantom COR-003 filed.
- **Out of scope (next session):** HANDOVER refresh (3 undeployed changes accumulated), then TST-001 + the 3 remaining Phase 4 RBAC tasks.

## 2026-05-28 — HANDOVER refresh post-Phase-4 Cluster-A closure + TOOL-COH-003 filing
- **Session ID:** 2026-05-28-handover-refresh-postphase4-cluster-a + TOOL-COH-003 filing
- **Tasks closed:** none — doc-refresh + filing session (precedent: HANDOVER refresh `bc917b4` 2026-05-28 post-prod-deploy + filings `c901d48`; same shape — filing commit FIRST, refresh commit second).
- **Tasks moved to BLOCKED:** none.
- **Tasks filed:** **TOOL-COH-003** (10 pre-existing `Closed_by`-format coherence-gate violations — non-blocking gate noise).
- **Commits:** `82d37e0` (file TOOL-COH-003 — BACKLOG.md only, no `[closes]`), `<pending>` (HANDOVER refresh + this entry).
- **Counter:** unchanged — checked-set **56 → 56** (no closure); coherence script raw DONE **57 → 57**. The +1 script-vs-checked-set gap is structural (DAT-003+DAT-004 bundle = 1 unit in checked-set, 2 in the script). Since last refresh (script 53 / checked-set 52): +4 from the COR-038/DOC-001/COR-001/COR-002 closures. BACKLOG header totals: 195 → **196** (+TOOL-COH-003 only; COR-001/COR-002 were already in the original 173 → no increment).
- **Bootstrap verification (ground-truth discipline):** all cited SHAs confirmed via `git log` — COR-038 fix `24c6929`/closeout `d204f0b`, DOC-001 fix `006adb7`/closeout `dd1381f`, COR-001 fix `cb3b5e1`/closeout `e3865c3`, COR-002 fix `27c0424`/closeout `60bd546`. Master HEAD = `60bd546`, prod = `ebcd9e1` (15 commits behind master; 3 of them code-runtime). All 4 closures confirmed `Status: DONE` + `Closed_by` in BACKLOG.
- **5 count/label corrections vs the invocation prompt's prose (every number re-derived, not transcribed):**
  1. **Cluster-letter collision** — prompt framed COR-001/002 as "Cluster A", COR-028/SEC-030 "B", TST-001/018 "C". BACKLOG `Cluster:` field marks **all six as `B`**. A/B/C is the kickoff thematic overlay, NOT the field. HANDOVER §Current-state now names the collision explicitly (field `B` = authority; A/B/C = overlay; the overlay's "B" ≠ the field's "B").
  2. **TOOL-COH-003 dual root-cause** — prompt's "backtick-format / fenced-code-blocks" guess was wrong. Re-extracted: 10 = **8 backtick-wrapped `Closed_by` SHAs** (DAT-032/033/034/036, COR-034/035, DAT-038, COR-037; gate's `.split()[0]` yields `` `7af1991` `` incl. backticks, fails `^[0-9a-f]{7,40}$`) + **2 stale leading `(none — …)` lines** (DAT-035, DAT-037, real SHA `148b713`/`128393e` in a later Learnings bullet). Filed with both sub-causes.
  3. **"+6 commits" → 15 commits / 3 runtime** — prod `ebcd9e1`..master `60bd546` = 15 commits; only 3 are code-runtime (`24c6929`/`cb3b5e1`/`27c0424`); DOC-001 `006adb7` is docs-only; rest are bookkeeping.
  4. **PROGRESS_LOG counter** — prompt said "52→52 / 53→53"; those were the *previous* refresh's numbers. Current (post-4-closures) = checked-set 56 / script 57. Confirmed by live script run ("Checked 57 DONE/VERIFIED").
  5. **Filings table off-by-one** — prompt said "14→15 total / 11 Phase-3-arc / 13 DONE". Actual table had 13 rows (10 Phase-3-arc, not 11). After +TOOL-COH-003 = **14 rows / 12 DONE + 2 TODO** (TOOL-DBSYNC-001, TOOL-COH-003). Fixed the inherited off-by-one.
- **Scope of refresh (surgical, structure + tone preserved):**
  (a) Title + §Current-state header → master `60bd546`, script 57 / checked-set 56; coherence-count note updated + a TOOL-COH-003 gate-noise note added.
  (b) NEW §"Phase 4 — RBAC complétude" subsection in Current-state: 2/6 done table (COR-001/002) + the cluster-letter-collision reconciliation.
  (c) §PROD-DEPLOY master-HEAD row: "functionally current" → "3 code-only runtime deltas pending deploy" (COR-038/COR-001/COR-002), no migration.
  (d) Operational carry-forward #2 (DAT-038): "trigger sole-line, 500 until COR-038 lands" → "COR-038 FIXED ON MASTER `24c6929`, STILL LIVE 500 on prod `ebcd9e1` until next deploy" (support lens preserved). #1/#3/#4 unchanged.
  (e) NEW §"Resolved carry-forwards (Phase-4 Cluster-A)": templateKey-only principle discharged on epics+milestones, remains forward-looking for the 4 remaining.
  (f) §Filings: COR-038 TODO→DONE `24c6929`, DOC-001 TODO→DONE `006adb7`, +TOOL-COH-003 (TODO); header recount to 14 / 12 DONE + 2 TODO with an explicit off-by-one correction note.
  (g) Process learnings #15 (`@Global()` discovery before module wiring — EpicsModule/MilestonesModule no-op confirmed) + #16 (AC#4 path-specific, never inherited from sibling — COR-002 imports audit infra yet stays N/A because the changed method is a read-gate).
  (h) §Next-session full rewrite: immediate cleanups cleared; the 3 GRAVED judgment points (COR-028 reachability nuance / TST-018 audit-presupposition moot / SEC-030 net-new `userReadWhere` design surface); sequence arguments (Cluster B before C; COR-028 vs SEC-030 ordering not pre-decided); 3 undeployed runtime deltas non-blocking.
  (i) §Preserve/unchanged updated to list Phase-3 mini-arc + Phase-4 Cluster A as preserved parts, +TOOL-COH-003 as a new known-debt member.
- **Files touched:** 3 — `BACKLOG.md` (TOOL-COH-003 entry + header total 195→196, commit `82d37e0`), `HANDOVER.md` (refresh), `PROGRESS_LOG.md` (this entry).
- **Gates:** no code touched → no `nest build` / `pnpm test` / `pnpm test:e2e` / migrate. Coherence script will still report the 10 pre-existing `Closed_by`-format violations post-commit — that is precisely the subject of TOOL-COH-003, deliberately NOT cleaned in this session (separate task).
- **Out of scope (next session):** Phase 4 Cluster B/C — pick per the 3 graved judgment points (start with COR-028 or SEC-030 per operator). Optional: micro-deploy the 3 runtime deltas, or bundle at first Phase-4 deploy.
