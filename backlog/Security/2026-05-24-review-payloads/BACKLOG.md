...
**Closed_by:** 2b23a19
**Learnings:**
REAL fix (multi-file scope authorized by operator). The audit's literal main.ts suggestion was sound for option 2 (a fastify onRequest hook), NOT for serving avatars through a guarded Nest controller alone — an `<img>` cannot carry a Bearer header, so the frontend had to change too. Implemented option 2: a new `common/fastify/uploads-auth.hook.ts` `onRequest` hook registered on the fastify instance BEFORE @fastify/static (main.ts), gating every `/api/uploads/` request on a valid Bearer access token via the app's configured `JwtService` (`app.get(JwtService)` — resolves at boot, confirmed by a live boot). Kept @fastify/static (so its `../` traversal confinement is retained — no hand-rolled controller path-traversal hole). Scope decision = **authenticated-only** (any valid access token), NOT a `users:read` gate: it closes the exact reported failure mode (anonymous access), matches the existing exposure (avatarUrl is already returned to every authenticated user), and avoids stripping EXTERNAL_PRESTATAIRE of their own avatar; signature+expiry only (no jti-blacklist/nbf — a logged-out-but-unexpired token revealing a profile photo for ≤access-TTL is negligible vs. the anon hole). Frontend: new `useAuthedAvatar` hook + `UserAvatar` now load uploaded avatars (`/api/uploads/*`) via the authenticated axios client → blob → object URL (preset SVGs / external URLs pass through unchanged); object URLs are cached at module scope keyed by avatarUrl and deliberately NOT revoked (the same avatar renders in many simultaneously-mounted components; cache is bounded by distinct avatars and cleared on the logout page reload). **Witness** (`uploads-auth.hook.spec.ts`): a real Fastify server + @fastify/static + `inject()` (no DB/Redis) — a "control" suite locks the failure mode (no hook → anon GET = 200), the fix suite asserts anon GET → 401 / valid Bearer → 200 / bad token → 401 / non-uploads route untouched. Stash-RED proven: neutering the hook flips the 3 auth-rejection assertions to `expected 200 to be 401`. AC#2 fail-pre is thus 200 (file served), not 404. Frontend witness added to `UserAvatar.test.tsx` (mocked api+createObjectURL → blob src; 401 → monogram fallback). AC#4 N/A (read-only file access, not in the audit-sensitive mutation list). No migration. Gate green: API `vitest` (2034) + web `jest` (654) + `nest build` + `pnpm build` + lint of the touched files (after the ajv-tooling fix). **Live smoke (dev API :4000, real JWT_SECRET):** anon GET `/api/uploads/avatars/<f>.png` → 401; bad Bearer → 401; valid forged Bearer + real file → 200; valid Bearer + missing file → 404 (auth passed, not blocked); `/api/health` (non-uploads) reached (hook ignores it). No deploy (LOCAL only). **Follow-up correctness fix (post-review): the hook ran BEFORE `enableCors()` and 401'd the CORS preflight — a cross-origin authed blob fetch (dev: web :4001 → api :4000, no Next rewrite) sends `Authorization`, so the browser fires an `OPTIONS` preflight FIRST, which carries no Bearer → 401 → the real GET is blocked → avatar silently falls back to initials (invisible in prod where `/api` is same-origin, and missed by curl/unit-mock/raw-fastify-inject). Fixed: the hook now returns early for `OPTIONS` (lets CORS answer the preflight); the real GET still requires a token. Witness gains an OPTIONS-not-401 case (7 tests). Live-verified: `OPTIONS /api/uploads/avatars/x.png` (Origin :4001, ACRH authorization) → 204 + `access-control-allow-origin: http://localhost:4001`; anon GET still 401.**
