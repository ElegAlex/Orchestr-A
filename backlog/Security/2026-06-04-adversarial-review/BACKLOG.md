...
- **2026-06-06 — DONE.** Audit-emit cluster slice 11 (FINAL; the genuinely non-mechanical one). Added `LEAVE_CREATED` (entityType `Leave`, schema `z.union([leaveAudit, securityEnvelope])` — dual-provenance like LEAVE_APPROVED). **Decided inline (deviates from the manifest's literal "auditPersistence for all 3 + auditService for selfApprove" to AVOID the flagged double-emit):** exactly ONE durable LEAVE_CREATED per creation — self-approve emits via `auditService.log(LEAVE_CREATED)` (the security-envelope dual-write, preserving finding #6's security-flow visibility; **relabelled from the mislabelled LEAVE_APPROVED**, enriched with `after`={status,leaveTypeId,targetUserId,selfApproved}); PENDING + declaredByManager emit via `auditPersistence.log(LEAVE_CREATED)` directly (rich `leaveAudit` payload {targetUserId, selfApproved:false, operation:'create'|'create_by_manager', after:{status,leaveTypeId,dates,declaredByManager}}). create() is SERIALIZABLE (DAT-024) → both emits are AFTER the tx (the selfApprove auditService.log was already post-tx; the new durable emit is awaited-after), the OBS-015 hash-chain constraint. AC#1 (durable LEAVE_CREATED for all 3 paths) + AC#2 (actor/target/status captured). Witnesses: updated the existing selfApprove test (LEAVE_APPROVED→LEAVE_CREATED + asserts NO double durable emit) + new durable-PENDING witness (RED-by-absence). Gate green: nest build + api vitest 2264 + lint 0-err + coherence. **→ Audit-emit cluster COMPLETE (13/13).**
