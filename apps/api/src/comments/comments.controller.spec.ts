...
describe('CommentsController', () => {
  ...
  describe('create', () => {
    ...
    it('should create a comment successfully', async () => {
      ...
      expect(mockCommentsService.create).toHaveBeenCalledWith('user-id-1', createCommentDto);
    ...
    it('should throw NotFoundException when task not found', async () => {
      ...
      await expect(controller.create('user-id-1', createCommentDto)).rejects.toThrow(
        NotFoundException
      );
    });
  });
  ...
  describe('findAll', () => {
    it('should return paginated comments', async () => {
      ...
      expect(mockCommentsService.findAll).toHaveBeenCalledWith(1, 10, undefined);
    });
    ...
    it('should filter by taskId', async () => {
      ...
      expect(mockCommentsService.findAll).toHaveBeenCalledWith(1, 10, 'task-id-1');
    });
  });
  ...
  describe('update', () => {
    ...
    it('should update a comment successfully when user is author', async () => {
      ...
      const result = await controller.update('comment-id-1', 'user-id-1', updateCommentDto);
    ...
    it('should throw ForbiddenException when user is not the author', async () => {
      mockCommentsService.update.mockRejectedValue(
        new ForbiddenException('Vous ne pouvez modifier que vos propres commentaires')
      );
  ...
  describe('remove', () => {
    it('should delete a comment when user is author', async () => {
      mockCommentsService.remove.mockResolvedValue({ message: 'Commentaire supprimé' });

      const result = await controller.remove('comment-id-1', 'user-id-1', 'CONTRIBUTEUR');
    ...
    it('should delete a comment when user is admin', async () => {
      ...
      const result = await controller.remove('comment-id-1', 'admin-id', 'ADMIN');
    ...
    it('should throw ForbiddenException when user is not author and not admin', async () => {
      mockCommentsService.remove.mockRejectedValue(
        new ForbiddenException('Vous ne pouvez supprimer que vos propres commentaires')
      );
