    ...
    const fetchTask = async () => {
      try {
        ...
        if (taskData.projectId) {
          ...
          try {
            const projectTasksData = await tasksService.getByProject(
              taskData.projectId,
            );
        ...
        const taskAssigneeIds =
          (taskData.assignees
            ?.map((a: TaskAssignee) => a.user?.id || a.userId)
    ...
    try {
      ...
      toast.error(
        axiosError.response?.data?.message ||
          "Erreur lors de la mise a jour des dependances",
      );
      console.error(err);
      // Reset to original
                  ...
                  <div className="space-y-2">
                    {!task.dependencies || task.dependencies.length === 0 ? (
                      ...
                      task.dependencies.map((dep) => {
                        const hasConflict =
                          dep.dependsOnTask?.endDate && task.startDate
                            ? new Date(task.startDate) <=
                              new Date(dep.dependsOnTask.endDate)
                            : false;
                        return (
                          <div
