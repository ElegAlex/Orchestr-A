...
function makeTask(overrides: Partial<GanttTaskRow> & { id: string }): GanttTaskRow {
  return {
...
describe('groupTasks — milestone', () => {
  ...
  it('puts tasks without milestoneId in "Sans jalon" group at the end', () => {
    const tasks = [
      makeTask({ id: '1' }),
      makeTask({ id: '2', milestoneId: 'm1', milestoneName: 'Alpha', startDate: d('2026-06-01') }),
    ];
    const groups = groupTasks(tasks, 'milestone');
  ...
  it('sorts named groups by earliest task startDate', () => {
    const tasks = [
      makeTask({ id: '1', milestoneId: 'm-late', milestoneName: 'Late', startDate: d('2026-06-01') }),
      makeTask({ id: '2', milestoneId: 'm-early', milestoneName: 'Early', startDate: d('2026-01-01') }),
      makeTask({ id: '3', milestoneId: 'm-mid', milestoneName: 'Mid', startDate: d('2026-03-01') }),
    ];
    const groups = groupTasks(tasks, 'milestone');
  ...
  it('sorts tasks within each group by startDate ascending', () => {
    const tasks = [
      makeTask({ id: 'b', milestoneId: 'm1', milestoneName: 'M1', startDate: d('2026-03-01') }),
      makeTask({ id: 'a', milestoneId: 'm1', milestoneName: 'M1', startDate: d('2026-01-01') }),
      makeTask({ id: 'c', milestoneId: 'm1', milestoneName: 'M1', startDate: d('2026-02-01') }),
    ];
  ...
  it('handles mixed grouped and ungrouped tasks', () => {
    const tasks = [
      makeTask({ id: '1', milestoneId: 'm1', milestoneName: 'Alpha', startDate: d('2026-02-01') }),
      makeTask({ id: '2', startDate: d('2026-01-01') }),
      makeTask({ id: '3', milestoneId: 'm2', milestoneName: 'Beta', startDate: d('2026-03-01') }),
...
describe('groupTasks — epic', () => {
  ...
  it('puts tasks without epicId in "Sans épopée" group at the end', () => {
    const tasks = [
      makeTask({ id: '1' }),
      makeTask({ id: '2', epicId: 'e1', epicName: 'Epic', startDate: d('2026-06-01') }),
    ];
    const groups = groupTasks(tasks, 'epic');
  ...
  it('sorts named groups by earliest task startDate', () => {
    const tasks = [
      makeTask({ id: '1', epicId: 'e-late', epicName: 'Late', startDate: d('2026-06-01') }),
      makeTask({ id: '2', epicId: 'e-early', epicName: 'Early', startDate: d('2026-01-01') }),
    ];
    const groups = groupTasks(tasks, 'epic');
  ...
  it('sorts tasks within each group by startDate ascending', () => {
    const tasks = [
      makeTask({ id: 'z', epicId: 'e1', epicName: 'E', startDate: d('2026-05-01') }),
      makeTask({ id: 'a', epicId: 'e1', epicName: 'E', startDate: d('2026-01-01') }),
    ];
  ...
  it('handles mixed grouped and ungrouped tasks', () => {
    const tasks = [
      makeTask({ id: '1', epicId: 'e1', epicName: 'A', startDate: d('2026-02-01') }),
      makeTask({ id: '2', startDate: d('2026-01-01') }),
    ];
    const groups = groupTasks(tasks, 'epic');
