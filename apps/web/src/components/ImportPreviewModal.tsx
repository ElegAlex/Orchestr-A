...
export function ImportPreviewModal({
  ...
  const t = useTranslations("common.import");
      ...
      <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] flex flex-col">
        ...
        <div className="px-6 py-4 bg-gray-50 border-b border-gray-200">
          <div className="grid grid-cols-5 gap-4">
            <div className="text-center">
              ...
              <p className="text-xs text-gray-600">{t("summary.total")}</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-green-600">
                {summary.valid}
              </p>
              <p className="text-xs text-gray-600">{t("summary.valid")}</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-yellow-600">
                {summary.warnings}
              </p>
              <p className="text-xs text-gray-600">{t("summary.warnings")}</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-orange-600">
                {summary.duplicates}
              </p>
              <p className="text-xs text-gray-600">{t("summary.duplicates")}</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-red-600">
                {summary.errors}
              </p>
              <p className="text-xs text-gray-600">{t("summary.errors")}</p>
            </div>
          </div>
        </div>
        ...
        <div className="px-6 border-b border-gray-200">
          <nav className="-mb-px flex space-x-4 overflow-x-auto">
            ...
            >
              {t("tabs.all", { count: summary.total })}
            </button>
            <button
            ...
            >
              {t("tabs.valid", { count: summary.valid })}
            </button>
            <button
            ...
            >
              {t("tabs.warnings", { count: summary.warnings })}
            </button>
            <button
            ...
            >
              {t("tabs.duplicates", { count: summary.duplicates })}
            </button>
            <button
            ...
            >
              {t("tabs.errors", { count: summary.errors })}
            </button>
          </nav>
        </div>
                  ...
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex items-center space-x-2">
                      <span className="text-sm font-medium text-gray-700">
                        {t("table.line", { lineNumber: item.lineNumber })}
                      </span>
            ...
            >
              {isImporting ? (
                <>
                  ...
                  <span>{t("footer.importing")}</span>
                </>
              ) : (
                <span>{t("footer.confirm", { count: importCount })}</span>
              )}
            </button>
          </div>
        </div>
