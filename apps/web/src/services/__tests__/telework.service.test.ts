...
describe("teleworkService", () => {
  ...
  const mockTeleworks = [
    mockTelework,
    { ...mockTelework, id: "tw-2", date: "2025-06-16" },
  ];
  ...
  describe("getByDateRange", () => {
    it("should fetch telework entries by date range", async () => {
      ...
      const result = await teleworkService.getByDateRange(
        "2025-06-01",
        "2025-06-30",
      );

      expect(api.get).toHaveBeenCalledWith(
        "/telework?startDate=2025-06-01&endDate=2025-06-30",
      );
      expect(result).toEqual(mockTeleworks);
    });
  ...
  describe("update", () => {
    it("should update a telework entry", async () => {
      ...
      const result = await teleworkService.update("tw-1", {
        isTelework: false,
      });

      expect(api.patch).toHaveBeenCalledWith("/telework/tw-1", {
        isTelework: false,
      });
