...
describe('leaveTypesService', () => {
  ...
  const mockLeaveTypes = [mockLeaveType, { ...mockLeaveType, id: 'lt-2', code: 'RTT', name: 'RTT' }];
  ...
  describe('update', () => {
    it('should update a leave type', async () => {
      ...
      const result = await leaveTypesService.update('lt-1', { name: 'Updated Leave Type' });

      expect(api.patch).toHaveBeenCalledWith('/leave-types/lt-1', { name: 'Updated Leave Type' });
    ...
    it('should update leave type active status', async () => {
      ...
      const result = await leaveTypesService.update('lt-1', { isActive: false });

      expect(api.patch).toHaveBeenCalledWith('/leave-types/lt-1', { isActive: false });
      expect(result).toEqual(updatedLeaveType);
    });
  });
  ...
  describe('delete', () => {
    ...
    it('should handle deactivation instead of deletion', async () => {
      const deleteResponse = { message: 'Leave type deactivated', deactivated: true };
      (api.delete as jest.Mock).mockResolvedValue({ data: deleteResponse });
  ...
  describe('reorder', () => {
    it('should reorder leave types', async () => {
      ...
      expect(api.post).toHaveBeenCalledWith('/leave-types/reorder', { orderedIds });
      expect(result).toEqual(mockLeaveTypes);
    });
  });
});
