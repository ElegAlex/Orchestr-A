...
describe("tasksService", () => {
  ...
  describe("getAll", () => {
    ...
    it("should combine all parameters", async () => {
      ...
      expect(api.get).toHaveBeenCalledWith(
        "/tasks?page=1&limit=20&status=TODO&priority=CRITICAL",
      );
    });
  });
  ...
  describe("getByDateRange", () => {
    it("should fetch tasks by date range", async () => {
      ...
      const result = await tasksService.getByDateRange(
        "2025-01-01",
        "2025-12-31",
      );
  ...
  describe("update", () => {
    it("should update a task", async () => {
      ...
      const result = await tasksService.update("task-1", {
        title: "Updated Task",
      });
  ...
  describe("updateProgress", () => {
    it("should update task progress", async () => {
      ...
      expect(api.patch).toHaveBeenCalledWith("/tasks/task-1/progress", {
        progress: 50,
      });
      expect(result).toEqual(updatedTask);
    });
  ...
  describe("addDependency", () => {
    it("should add a task dependency", async () => {
      ...
      expect(api.post).toHaveBeenCalledWith("/tasks/task-1/dependencies", {
  ...
  describe("removeDependency", () => {
    it("should remove a task dependency", async () => {
      ...
      expect(api.delete).toHaveBeenCalledWith(
        "/tasks/task-1/dependencies/dep-1",
      );
    });
  });
