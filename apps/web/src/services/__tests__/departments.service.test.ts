...
describe('departmentsService', () => {
  ...
  const mockDepartments = [mockDepartment, { ...mockDepartment, id: 'dept-2', name: 'HR Department' }];
  ...
  describe('getAll', () => {
    it('should fetch all departments from wrapped response', async () => {
      (api.get as jest.Mock).mockResolvedValue({ data: { data: mockDepartments } });
  ...
  describe('update', () => {
    it('should update a department', async () => {
      ...
      const result = await departmentsService.update('dept-1', { name: 'Updated Department' });

      expect(api.patch).toHaveBeenCalledWith('/departments/dept-1', { name: 'Updated Department' });
