...
describe("skillsService", () => {
  ...
  const mockSkills = [
    mockSkill,
    { ...mockSkill, id: "skill-2", name: "React" },
  ];
  ...
  describe("update", () => {
    it("should update a skill", async () => {
      ...
      const result = await skillsService.update("skill-1", {
        name: "Updated Skill",
      });

      expect(api.patch).toHaveBeenCalledWith("/skills/skill-1", {
        name: "Updated Skill",
      });
  ...
  describe("removeFromUser", () => {
    it("should remove a skill from a user", async () => {
      ...
      expect(api.delete).toHaveBeenCalledWith(
        "/skills/user/user-1/remove/skill-1",
      );
    });
  });
  ...
  describe("getMatrix", () => {
    ...
    it("should fetch skills matrix with filters", async () => {
      ...
      expect(api.get).toHaveBeenCalledWith(
        "/skills/matrix?departmentId=dept-1&category=TECHNICAL",
      );
    });
  });
  ...
  describe("findUsersBySkill", () => {
    ...
    it("should find users by skill with min level", async () => {
      ...
      expect(api.get).toHaveBeenCalledWith(
        "/skills/search/skill-1?minLevel=EXPERT",
      );
    });
  });
  ...
  describe("updateUserSkill", () => {
    it("should update user skill level", async () => {
      ...
      const result = await skillsService.updateUserSkill("user-1", "skill-1", {
        level: SkillLevel.MASTER,
      });

      expect(api.patch).toHaveBeenCalledWith(
        "/skills/user/user-1/skill/skill-1",
        { level: SkillLevel.MASTER },
      );
