...
describe("leavesService", () => {
  ...
  describe("getAll", () => {
    ...
    it("should filter by userId", async () => {
      ...
      expect(api.get).toHaveBeenCalledWith(
        "/leaves?page=1&limit=100&userId=user-1",
      );
    });
    ...
    it("should filter by status", async () => {
      ...
      expect(api.get).toHaveBeenCalledWith(
        "/leaves?page=1&limit=100&status=APPROVED",
      );
    });
  ...
  describe("getByDateRange", () => {
    it("should fetch leaves by date range", async () => {
      ...
      const result = await leavesService.getByDateRange(
        "2025-01-01",
        "2025-12-31",
      );
  ...
  describe("update", () => {
    it("should update a leave request", async () => {
      ...
      const result = await leavesService.update("leave-1", {
        endDate: "2025-06-10",
      });

      expect(api.patch).toHaveBeenCalledWith("/leaves/leave-1", {
        endDate: "2025-06-10",
      });
  ...
  describe("updateStatus", () => {
    it("should update leave status", async () => {
      ...
      expect(api.patch).toHaveBeenCalledWith("/leaves/leave-1/status", {
  ...
  describe("approve", () => {
    it("should approve a leave request", async () => {
      ...
      expect(api.post).toHaveBeenCalledWith("/leaves/leave-1/approve", {
        comment: "Approved",
      });
  ...
  describe("reject", () => {
    it("should reject a leave request", async () => {
      ...
      expect(api.post).toHaveBeenCalledWith("/leaves/leave-1/reject", {
  ...
  describe("createDelegation", () => {
    it("should create a validation delegation", async () => {
      ...
      const result = await leavesService.createDelegation(
        "user-2",
        "2025-06-01",
        "2025-06-10",
      );
  ...
  describe("deactivateDelegation", () => {
    it("should deactivate a delegation", async () => {
      ...
      expect(api.delete).toHaveBeenCalledWith(
        "/leaves/delegations/delegation-1",
      );
    });
  });
});
