...
describe('planning-utils', () => {
  describe('getServiceStyle', () => {
    it('should return blue for development-related services', () => {
      expect(getServiceStyle('Développement')).toEqual({ icon: '', color: 'blue' });
      expect(getServiceStyle('Dev Team')).toEqual({ icon: '', color: 'blue' });
      expect(getServiceStyle('Informatique')).toEqual({ icon: '', color: 'blue' });
      expect(getServiceStyle('Technique')).toEqual({ icon: '', color: 'blue' });
    });

    it('should return emerald for admin/finance services', () => {
      expect(getServiceStyle('Administration')).toEqual({ icon: '', color: 'emerald' });
      expect(getServiceStyle('Gestion')).toEqual({ icon: '', color: 'emerald' });
      expect(getServiceStyle('Finance')).toEqual({ icon: '', color: 'emerald' });
    });

    it('should return purple for communication/marketing services', () => {
      expect(getServiceStyle('Communication')).toEqual({ icon: '', color: 'purple' });
      expect(getServiceStyle('Marketing')).toEqual({ icon: '', color: 'purple' });
    });

    it('should return pink for HR services', () => {
      expect(getServiceStyle('RH')).toEqual({ icon: '', color: 'pink' });
      expect(getServiceStyle('Ressources Humaines')).toEqual({ icon: '', color: 'pink' });
    });

    it('should return slate for legal services', () => {
      expect(getServiceStyle('Juridique')).toEqual({ icon: '', color: 'slate' });
    ...
    it('should return cyan for support services', () => {
      ...
      expect(getServiceStyle('Assistance')).toEqual({ icon: '', color: 'cyan' });
    });
    ...
    it('should return gray for unknown services', () => {
      ...
      expect(getServiceStyle('Random Service')).toEqual({ icon: '', color: 'gray' });
    });
