...
test.describe('Full User Workflow', () => {
  test('complete workflow: login → dashboard → projects → tasks → logout', async ({ page }) => {
    // 1. Login
    ...
    const logoutButton = page.locator('button:has-text("Déconnexion"), button:has-text("Logout"), [aria-label="Déconnexion"]').first();
