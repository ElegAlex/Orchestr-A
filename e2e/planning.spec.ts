...
test.describe('Planning View', () => {
  ...
  test('should display week view by default', async ({ page }) => {
    ...
    const weekButton = page.locator('button:has-text("Semaine"), [data-view="week"]').first();
    const isWeekActive = await weekButton.getAttribute('class').then(c => c?.includes('active') || c?.includes('selected')).catch(() => false);

    // Ou vérifier la présence d'une grille avec 7 colonnes (jours de la semaine)
    const weekGrid = await page.locator('[data-testid="planning-grid"], .planning-grid').isVisible().catch(() => false);
  ...
  test('should switch between week and month view', async ({ page }) => {
    ...
    const monthButton = page.locator('button:has-text("Mois"), [data-view="month"]').first();
    ...
    if (await monthButton.isVisible()) {
      ...
      const monthGrid = await page.locator('[data-view="month"], .month-view, [data-testid="month-grid"]').isVisible().catch(() => false);
  ...
  test('should navigate to previous/next period', async ({ page }) => {
    ...
    const currentPeriod = await page.locator('h2, .period-title, [data-testid="current-period"]').first().textContent();

    // Cliquer sur le bouton suivant
    const nextButton = page.locator('button:has-text("Suivant"), button[aria-label="Suivant"], button:has(svg)').last();
    ...
    if (await nextButton.isVisible()) {
      ...
      const newPeriod = await page.locator('h2, .period-title, [data-testid="current-period"]').first().textContent();
  ...
  test('should display users in planning grid', async ({ page }) => {
    ...
    const userRows = page.locator('[data-testid="user-row"], .user-row, tr[data-user-id]');
  ...
  test('should display telework indicators', async ({ page }) => {
    ...
    const teleworkIndicators = page.locator('[data-testid="telework-indicator"], .telework, [data-telework="true"]');
  ...
  test('should display leave indicators', async ({ page }) => {
    ...
    const leaveIndicators = page.locator('[data-testid="leave-indicator"], .leave, [data-leave="true"]');
  ...
  test('should toggle telework status', async ({ page }) => {
    ...
    const teleworkCell = page.locator('[data-testid="telework-cell"], .telework-toggle').first();
  ...
  test('should display task in planning', async ({ page }) => {
    ...
    const tasks = page.locator('[data-testid="task-item"], .task-badge, [data-task-id]');
  ...
  test('should open task details modal', async ({ page }) => {
    ...
    const taskItem = page.locator('[data-testid="task-item"], .task-badge').first();

    if (await taskItem.isVisible()) {
      await taskItem.click();

      // Vérifier que le modal de détails s'ouvre
      await expect(page.locator('[role="dialog"], .modal, .task-details')).toBeVisible({ timeout: 5000 });
    }
  });
  ...
  test('should drag and drop task', async ({ page }) => {
    ...
    const taskItem = page.locator('[draggable="true"], [data-testid="draggable-task"]').first();

    if (await taskItem.isVisible()) {
      // Trouver une cellule cible
      const targetCell = page.locator('[data-testid="drop-zone"], .planning-cell').nth(5);
...
test.describe('Planning - Filters and Search', () => {
  ...
  test('should filter planning by department', async ({ page }) => {
    ...
    const deptFilter = page.locator('select:has-text("Département"), [data-testid="department-filter"]').first();
  ...
  test('should search users in planning', async ({ page }) => {
    ...
    const searchInput = page.locator('input[type="search"], input[placeholder*="Rechercher"], [data-testid="user-search"]').first();
  ...
  test('should go to current week/today', async ({ page }) => {
    ...
    const nextButton = page.locator('button:has-text("Suivant"), button[aria-label="Suivant"]').first();
    if (await nextButton.isVisible()) {
      await nextButton.click();
      await nextButton.click();
      await page.waitForTimeout(300);
    ...
    const todayButton = page.locator('button:has-text("Aujourd\'hui"), button:has-text("Courante"), [data-testid="today-button"]').first();
