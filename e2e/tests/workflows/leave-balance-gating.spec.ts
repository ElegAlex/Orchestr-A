...
test.describe("Wave 5 — uniform leave balance gating (cross-year + audit)", () => {
  ...
  test(
    "A. ADMIN self-approves Dec 28 → Jan 8 with both years funded",
    async ({ asRole }) => {
      const adminPage = await asRole("admin");
  ...
  test(
    "B. ADMIN edits a CANCELLED leave — gate must permit (CANCELLED is filtered)",
    async ({ asRole }) => {
      ...
      const editRes = await adminPage.request.patch(
  ...
  test(
    "C. CONTRIBUTEUR with zero allocation in destination year is rejected with the failing year named",
    async ({ asRole }) => {
      const adminPage = await asRole("admin");
      const contributeurPage = await asRole("contributeur");
