...
test.describe("Leave Lifecycle", () => {
  ...
  test(
    ...
    async ({ request }) => {
      ...
      expect(
        leaveId,
        "La réponse de création doit contenir un id",
      ).toBeTruthy();
      expect(
        expectedDelta,
  ...
  // ─────────────────────────────────────────────────────────────────────────
  test("manager rejette un congé CP → contributeur voit le solde used inchangé (balance non débitée)", async ({
    request,
  }) => {
    ...
    const balBefore = await request.get(`${baseURL}/api/leaves/me/balance`, {
      headers: authHeaders(contributeurToken),
    });
    expect(balBefore.ok(), `GET balance failed: ${balBefore.status()}`).toBe(
      true,
    );
    ...
    const rejectRes = await request.post(
      `${baseURL}/api/leaves/${leaveId}/reject`,
      {
        headers: authHeaders(adminToken),
        data: {
          reason: "TST-024 test rejection — solde ne doit pas être débité",
        },
      },
    );
    expect(
    ...
    const balAfter = await request.get(`${baseURL}/api/leaves/me/balance`, {
      headers: authHeaders(contributeurToken),
    });
    expect(
      balAfter.ok(),
  ...
  // ─────────────────────────────────────────────────────────────────────────
  test("admin approuve un congé CP puis l'annule → solde used restauré à la valeur pré-approbation", async ({
    request,
  }) => {
    const baseURL = test.info().project.use.baseURL ?? "http://localhost:4001";
  ...
  // ─────────────────────────────────────────────────────────────────────────
  test("manager délègue sa validation à l'admin → l'admin-délégué approuve un congé CP → solde débité", async ({
    request,
  }) => {
    const baseURL = test.info().project.use.baseURL ?? "http://localhost:4001";
    ...
    await request.delete(`${baseURL}/api/leaves/delegations/${delegationId}`, {
      headers: authHeaders(managerToken),
    });
  });
  ...
  // ─────────────────────────────────────────────────────────────────────────
  test("manager de département inactif → fallback MANAGE_ANY (admin) approuve → solde débité", async ({
    request,
  }) => {
    const baseURL = test.info().project.use.baseURL ?? "http://localhost:4001";
    ...
    try {
      ...
      expect(balAfter.ok(), "GET balance après approbation fallback").toBe(
        true,
      );
      const cpAfter = (
        (await balAfter.json()).byType as Array<{
