...
test.describe("UI — Administration des rôles (/fr/admin/roles)", () => {
  ...
  test("Onglet Templates : 26 templates read-only, aucun bouton 'Éditer'", async ({
    asRole,
  }) => {
    const page = await asRole("admin");
  ...
  test("Onglet Templates : 9 chips catégories et filtrage fonctionnel", async ({
    asRole,
  }) => {
    const page = await asRole("admin");
    await gotoTemplatesTab(page);
  ...
  test("Onglet Templates : click sur card ouvre modale détail avec bannière read-only", async ({
    asRole,
  }) => {
    const page = await asRole("admin");
    await gotoTemplatesTab(page);
  ...
  test("Onglet Rôles : aucun rôle système exposé + grouping par template utilisé", async ({
    asRole,
  }) => {
    const page = await asRole("admin");
    await gotoRolesAdmin(page);
    ...
    await expect(page.locator("[data-testid='roles-show-system']")).toHaveCount(
      0,
    );
  });

  test("Onglet Rôles : état vide affiche le message d'amorçage quand aucun rôle créé", async ({
    asRole,
  }) => {
    const page = await asRole("admin");
    await gotoRolesAdmin(page);
  ...
  test("Onglet Rôles : ADMIN peut créer + renommer + supprimer un rôle éditable", async ({
    asRole,
  }) => {
    const page = await asRole("admin");
    await gotoRolesAdmin(page);
    ...
    await expect(page.locator("text=/créé|enregistré/i").first()).toBeVisible({
      timeout: 10000,
    });

    // Isoler par code unique (selector stable, pas sensible au re-render).
    const rowByCode = page.locator("[data-testid='role-row']").filter({
      has: page.locator(`span.font-mono:has-text("${uniqueCode}")`),
    });
    ...
    await expect(rowByCode.locator(`text="${renamedLabel}"`)).toBeVisible({
      timeout: 10000,
    });
    ...
    await expect(page.locator("text=/supprimé/i").first()).toBeVisible({
      timeout: 10000,
    });
  ...
  test("BASIC_USER (contributeur) : navigation vers /admin/roles → 403 ou redirect dashboard", async ({
    asRole,
  }) => {
