...
test.describe('Security — auth hardening (SEC-03/04/05)', () => {
  test(
    'SEC-03 — tampering auth_user_display does not grant admin privileges @smoke',
  ...
  test(
    'SEC-05 — login throttle: 6th rapid wrong attempt returns 429 @smoke',
  ...
  test(
    'SEC-04 — logout blacklists access token; subsequent /auth/me returns 401 @smoke',
    async ({ request, baseURL }) => {
  ...
  test(
    'SEC-04 — refresh rotation: replaying old refresh triggers reuse detection @smoke',
    async ({ request, baseURL }) => {
