...
test.describe('Leaves Management', () => {
  ...
  test('should display leaves list', async ({ page }) => {
    ...
    const hasList = await page.locator('table, [data-testid="leaves-list"]').isVisible().catch(() => false);
    const hasEmptyMessage = await page.locator('text=/aucun|vide|pas de congé/i').isVisible().catch(() => false);
  ...
  test('should open new leave request form', async ({ page }) => {
    ...
    const newLeaveButton = page.locator('button:has-text("Nouveau"), button:has-text("Demander"), button:has-text("Ajouter")').first();
  ...
  test('should create a leave request', async ({ page }) => {
    ...
    if (await newLeaveButton.isVisible()) {
      ...
      // Attendre le modal
      await page.waitForSelector('[role="dialog"], form, .modal', { timeout: 5000 });
      ...
      // Soumettre
      const submitButton = page.locator('button[type="submit"], button:has-text("Soumettre"), button:has-text("Créer"), button:has-text("Envoyer")').first();
      if (await submitButton.isVisible()) {
        await submitButton.click();

        // Vérifier le succès
        await expect(page.locator('text=/succès|créé|envoyé/i')).toBeVisible({ timeout: 5000 }).catch(() => {});
      }
    }
  });
  ...
  test('should filter leaves by status', async ({ page }) => {
    ...
    const statusFilter = page.locator('select:has-text("Statut"), [data-testid="status-filter"]').first();

    if (await statusFilter.isVisible()) {
      await statusFilter.click();
      // Sélectionner "En attente" ou similaire
      await page.locator('[role="option"]:has-text("attente"), option:has-text("attente")').first().click().catch(() => {});
    }
  });
  ...
  test('should display leave details', async ({ page }) => {
    ...
    const firstLeave = page.locator('tr[data-testid="leave-item"], [data-testid="leave-card"]').first();

    if (await firstLeave.isVisible()) {
      await firstLeave.click();

      // Vérifier que les détails s'affichent
      await expect(page.locator('[role="dialog"], .modal, .leave-details')).toBeVisible({ timeout: 5000 }).catch(() => {});
    }
  });
});
...
test.describe('Leave Approval Workflow', () => {
  ...
  test('should display pending leaves for approval', async ({ page }) => {
    ...
    const pendingFilter = page.locator('button:has-text("En attente"), [data-testid="pending-filter"]').first();
    if (await pendingFilter.isVisible()) {
      await pendingFilter.click();
    }

    // Vérifier la présence de congés en attente ou message
    const hasPendingLeaves = await page.locator('text=/pending|en attente/i').isVisible().catch(() => false);
    const hasEmptyMessage = await page.locator('text=/aucun|vide/i').isVisible().catch(() => false);
  ...
  test('should approve a leave request', async ({ page }) => {
    ...
    const pendingLeave = page.locator('[data-status="PENDING"], tr:has-text("attente")').first();

    if (await pendingLeave.isVisible()) {
      // Cliquer sur le bouton d'approbation
      const approveButton = pendingLeave.locator('button:has-text("Approuver"), button:has-text("Valider")').first();

      if (await approveButton.isVisible()) {
        await approveButton.click();

        // Confirmer si nécessaire
        const confirmButton = page.locator('[role="dialog"] button:has-text("Confirmer"), [role="dialog"] button:has-text("Oui")').first();
        if (await confirmButton.isVisible()) {
          await confirmButton.click();
        }

        // Vérifier le succès
        await expect(page.locator('text=/approuvé|validé|succès/i')).toBeVisible({ timeout: 5000 }).catch(() => {});
      }
    }
  });
  ...
  test('should reject a leave request', async ({ page }) => {
    ...
    if (await pendingLeave.isVisible()) {
      ...
      const rejectButton = pendingLeave.locator('button:has-text("Refuser"), button:has-text("Rejeter")').first();
      ...
      if (await rejectButton.isVisible()) {
        ...
        // Confirmer
        const confirmButton = page.locator('[role="dialog"] button:has-text("Confirmer"), [role="dialog"] button:has-text("Refuser")').first();
        if (await confirmButton.isVisible()) {
          await confirmButton.click();
        }

        // Vérifier le succès
        await expect(page.locator('text=/refusé|rejeté|succès/i')).toBeVisible({ timeout: 5000 }).catch(() => {});
      }
    }
  });
});
