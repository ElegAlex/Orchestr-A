...
test.describe('Role-Based Access Control', () => {
  test.describe('Admin Role', () => {
    ...
    test('admin should see user management options', async ({ page }) => {
      ...
      const createButton = page.locator('button:has-text("Nouveau"), button:has-text("Créer"), button:has-text("Ajouter")').first();
      await expect(createButton).toBeVisible();

      // Vérifier la présence des actions (edit, delete)
      const actionButtons = page.locator('button[aria-label="Modifier"], button[aria-label="Supprimer"], button:has(svg)');
    ...
    test('admin should see all menu items in sidebar', async ({ page }) => {
      ...
      for (const item of menuItems) {
        const menuLink = page.locator(`nav a:has-text("${item}"), nav button:has-text("${item}")`).first();
  ...
  test.describe('Regular User Role (Contributeur)', () => {
    ...
    test('regular user should access basic pages', async ({ page }) => {
      ...
      const loginFailed = await page.locator('text=/incorrect|invalide|erreur/i').isVisible({ timeout: 3000 }).catch(() => false);
      ...
      const allowedPages = ['/dashboard', '/projects', '/tasks', '/profile', '/leaves', '/telework'];
    ...
    test('regular user should not see admin-only buttons', async ({ page }) => {
      ...
      const createUserButton = page.locator('button:has-text("Créer utilisateur"), button:has-text("Nouvel utilisateur")').first();

      // Soit le bouton n'est pas visible, soit on est redirigé
      const isVisible = await createUserButton.isVisible().catch(() => false);
      const isRedirected = page.url().includes('/403') || page.url().includes('/dashboard');
  ...
  test.describe('Unauthenticated User', () => {
    ...
    test('should access register page without redirection', async ({ page }) => {
...
test.describe('Role-Specific Features', () => {
  ...
  test('should display role in user profile', async ({ page }) => {
    ...
    const roleIndicator = page.locator('text=/admin|responsable|manager|contributeur/i');
  ...
  test('admin should see delete buttons', async ({ page }) => {
    ...
    const deleteButtons = page.locator('button[aria-label*="Supprimer"], button:has-text("Supprimer"), button:has(svg[class*="trash"]), .delete-button');
  ...
  test('should show appropriate actions based on resource ownership', async ({ page }) => {
    await page.goto('/projects');
    await page.waitForLoadState('networkidle');

    // Cliquer sur un projet
    const projectCard = page.locator('[data-testid="project-card"], .project-item, tr[data-project-id]').first();
