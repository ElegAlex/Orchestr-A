# Reconstruire le bundle « PLC-OFS en boîte »

Procédure pour régénérer la **simulation transportable** (OFS Tracker dans l'OS PLC, données
...
- **Sauvegarde OFS** (avec secrets) : `scripts/ofs/ofs-backup.sh` → `backup.tar.gz` (+ `.sha256`).
...
3. **Bundle ISO9660** (image + repo RPM + backup + `scripts/ofs/{ofs-lib,ofs-restore}.sh` + `ofs.conf` + `docker-compose.offline.yml` + `aio.env`) :
...
5. **Injecter le provisionnement** : `cp plc8.qcow2 plc8-ofs.qcow2 && virt-customize -a plc8-ofs.qcow2 --firstboot firstboot-provision.sh --selinux-relabel`
6. **Booter une fois** : `plc8-ofs.qcow2` (sda) + `scratch.iso` (sdb) + `docker-data.qcow2` (sdc), `q35` + **OVMF SB-OFF**, **disque SATA** (`ich9-ahci`/`ide-hd`), **sans réseau** → provisionne puis arrêt propre.
7. **Packager** : `plc8-ofs.qcow2` + `docker-data.qcow2` + `OVMF_CODE.fd`/`OVMF_VARS.fd` + `run-plc.sh` + `README.md`.
