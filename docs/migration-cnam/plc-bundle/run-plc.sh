#!/usr/bin/env bash
# =============================================================================
# run-plc.sh — Lance la simulation PLC-OFS (OFS Tracker dans l'OS PLC, air-gap).
...
echo "PLC-OFS — démarrage…"
echo "  -> App : http://localhost:$PORT  (patiente ~1-2 min après le login PLC)"
echo "  -> Quitter qemu : Ctrl-A puis X"
exec qemu-system-x86_64 -name plc-ofs -machine q35 -enable-kvm -m "$RAM" -smp 2 \
  -drive if=pflash,format=raw,readonly=on,file="$OVMF_CODE" \
  -drive if=pflash,format=raw,file="$HERE/.OVMF_VARS_run.fd" \
  -device ich9-ahci,id=ahci0 \
  -drive if=none,id=d0,format=qcow2,file="$HERE/plc8-ofs.qcow2"    -device ide-hd,bus=ahci0.0,drive=d0 \
