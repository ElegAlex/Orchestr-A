...
**Preuve que ça aboutit** (résultat final de mars) : 44 users / 21 projets / 185 tâches, 5 services `RUNNING`, conteneur `healthy`. Le mécanisme actuel rejoue ça **en une passe, automatiquement**, et ajoute la **preuve zéro-perte** (comptages + empreinte d'audit) que la session de mars n'avait pas. *(Note version : l'image est passée de PG16 — mars — à PG18 — aujourd'hui ; on restaure toujours dans une version PostgreSQL **≥** celle de la source, jamais inférieure.)*
